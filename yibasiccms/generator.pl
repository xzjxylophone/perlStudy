...
my $entityModuleName = $Config->{section}->{entityModuleName};
my $daoModuleName = $Config->{section}->{daoModuleName};
...
if (open(GENERATORFILE, $generatorXmlFile)) {
    while ($line = <GENERATORFILE>) {
        ...
        if ($line =~ /table tableName="([^"]+)" domainObjectName="([^"]+)" /) {
            ...
            my $outputEntityFileDir = $startDir."/".$entityModuleName."/src/main/java/".$replacePackageDir."/entity";
            ...
            my $outputMapperFileDir = $startDir."/".$entityModuleName."/src/main/resources/".$replacePackageDir."/entity/mapping";
